...
exports.getProperty = async (req, res) => {
    ...
    try {
        if (req.query.title) {
            ...
            data = await Propertymodel.find({ property_type: req.query.property_type })
...
exports.getSingleProperty = async (req, res) => {
    const result = await Propertymodel.findById(req.params.propertyId).populate('owner')
    res.json({ result })
}
...
exports.updateListing = async (req, res) => {
    const data = req.body
    const result = Propertymodel.findByIdAndUpdate(req.params.propertyId, data, function (err, docs) {
        if (!err) {
...
exports.addRoom = async (req, res) => {
    ...
    rooms.push(data)
    const result = Propertymodel.findByIdAndUpdate(req.params.propertyId, { rooms: rooms }, function (err, docs) {
        if (!err) {
...
exports.commentOnProperty = async (req, res) => {
    try {
        const comment_ = await new Commentmodel(req.body);
        await comment_.save()
...
exports.markRented = async (req, res) => {
    ...
    if (property.available) {
        const rent = new rentings({ property: propertyId, owner: req.userInfo._id, customer: customerId })
        rent.save()

        Propertymodel.findByIdAndUpdate(propertyId, { available: false }).then(function (err, docs) {
            if (!err) {
                console.log(docs)
            }
        })
...
exports.rentingsVendor = async (req, res) => {
    const rentings_ = await rentings.find({ owner: req.userInfo._id }).populate('owner').populate('property').populate('customer')
    res.json({ rentings: rentings_, success: true })
}

exports.rentingsCustomer = async (req, res) => {
    const rentings_ = await rentings.find({ customer: req.userInfo._id }).populate('customer', "property").populate('customer')
    res.json({ rentings: rentings_, success: true })
}
